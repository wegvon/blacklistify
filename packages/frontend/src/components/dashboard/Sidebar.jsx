  ...
  return (
    <aside className='bg-slate-900 w-64 p-4 flex flex-col text-white border-r border-slate-800'>
      <div className='flex items-center gap-3 px-2 py-3'>
        ...
        <div>
          <p className='text-slate-100 text-lg font-semibold leading-none'>Blacklistify</p>
          <p className='text-slate-400 text-xs mt-1'>DNSBL Monitoring</p>
        </div>
      </div> 
      <div className='flex-1 py-8 flex flex-col gap-1'>
        {DASHBOARD_SIDEBAR_LINKS.map((item) => (
          item.divider
            ? <div key={item.key} className='text-xs text-slate-500 uppercase tracking-wider px-3 pt-4 pb-1'>{item.label.replace(/---/g, '').trim()}</div>
