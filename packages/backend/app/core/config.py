...
class Settings:
    app_name: str = os.getenv("APP_NAME", "Blacklistify API")
    ...
    database_url: str = os.getenv("DATABASE_URL", os.getenv("SUPABASE_DB_URL", "sqlite:///./app.db"))
    cors_allowed_origins: list[str] = None  # type: ignore[assignment]

    # Supabase (Ripefy shared DB — read-only for public schema)
    ...
    default_admin_email: str = os.getenv("DEFAULT_ADMIN_EMAIL", "admin@blacklistify.local")
    ...
    scan_cache_ttl_hours: int = int(os.getenv("SCAN_CACHE_TTL_HOURS", "6"))
